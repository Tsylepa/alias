export const getTime = (state) => state.game.time;

export const getWinScore = (state) => state.game.winScore;

export const getTimeIsUp = (state) => state.game.timeIsUp;

export const getIsPlaying = (state) => state.game.playing;

export const getWordsCollection = (state) => state.game.wordsCollection;

export const getRoundResults = (state) => state.game.roundResults;

export const getScreen = (state) => state.game.screen;

export const getTeams = (state) => state.game.teams;

export const getCurrentTeam = (state) => state.game.currentTeam;

export const getCurrentGame = (state) => state.game.currentGame;

export const getModal = (state) => state.game.modal;

export const getCategory = (state) => state.game.category;

export const getLanguage = (state) => state.game.language;
